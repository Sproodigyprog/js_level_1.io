...
class Card {
  ...
  _closeModalSlider() {
    ...
    const checkCart = document.querySelector('.cart').style.display
  ...
  _renderCard() {
    ...
    const { _name, _brand, _color, _price } = this._product
    ...
    if (catalog) {
      ...
      this._name.innerHTML = `<b>Name: </b>${_name}`
      this._brand.innerHTML = `<b>Brand: </b>${_brand}`
      this._color.innerHTML = `<b>Color: </b>${_color}`
      this._price.innerHTML = `<b>Price: </b>${_price}`
      ...
      this._card.appendChild(this._addImg(_name))
  ...
  purchase() {
    ...
    if (!CartInstance.getCart.length) {
      ...
      const matches = CartInstance.getCart.map((product, ind, arr) => {
        return product.name
      })
      const cartLocal = CartInstance.getCart
    ...
    document.cookie = `cart=${JSON.stringify(CartInstance.getCart)}; sameSite=strict; Secure`
    CartInstance._renderCart()
  }
}
...
class Cart {
  ...
  _createShowCartBtn() {
    ...
    btn.classList.add('show-cart', 'cart--btn')
    btn.innerHTML = 'cart'
    btn.addEventListener('click', this._showCart.bind(this))
  ...
  _createCollapseCartBtn() {
    ...
    btn.classList.add('cart--nav-btn', 'cart--collapse-btn')
    btn.href = '#'
    btn.innerHTML = '\u21E9'
  ...
  _createHiddenCartBtn() {
    const btn = document.createElement('a')

    btn.classList.add('cart--nav-btn', 'cart--hidden-btn')
    btn.href = '#'
    btn.innerHTML = '\u2717'
    btn.addEventListener('click', this._hiddenCart.bind(this))
  ...
  _addProductAmount(productInd) {
    ...
    setTimeout(this._renderCart.bind(this), 100)
  ...
  _renderCart() {
    ...
    nav.appendChild(this._createCollapseCartBtn())
    nav.appendChild(this._createHiddenCartBtn())

    nav.classList.add('cart--nav')
